...
use hwfq::{Pkt, Scheduler, scheduler::htb::RateCounter};
...
impl<L: std::io::Write> Scheduler for TrafficPolicer<L> {
    fn enq(&mut self, p: Pkt) -> Result<(), Report> {
        ...
        self.tb.accumulate();
        if p.len() > self.tb.accum_bytes as usize || p.len() + tot_curr_len_bytes > self.max_len_bytes {
...
mod tests {
    ...
    fn pkt_parse() -> Pkt {
        let buf = vec![
            ...
            64, 17, 0x00, 0x00, 192, 168, 0, 1, 192, 168, 0, 2,
            // UDP Header
    ...
    fn test_rate() {
        ...
        let mut tp = TrafficPolicer::new(
            format!("--max_len_bytes=18000 --rate_bytes_per_sec={}", rate_bytes_per_sec)
        ).unwrap();
